...
A static site generator that converts Markdown files into a professional
documentation website with AJAX navigation.

Features:
    - YAML frontmatter parsing for page metadata
    - Hierarchical navigation (unlimited depth)
    - Automatic breadcrumb generation
    - Number prefix stripping for clean display names
    - Acronym capitalization (API, SQL, HTML, etc.)
    - FontAwesome icons per category
    - Syntax highlighting via Prism.js
    - Dark/light theme support
    - Bookmark and recent items persistence
    - Full-text search with highlighting
    - Mobile-responsive design
    - HTML export functionality
    - Configurable SEO and footer via config.json
...
# Words to keep lowercase (unless first word) - English grammar, not configurable
...
def capitalize_with_acronyms(text: str, acronyms: set = None) -> str:
    """
    Capitalize text with proper handling of acronyms.
    'api testing' -> 'API Testing' (if 'api' in acronyms)
    'sql queries' -> 'SQL Queries' (if 'sql' in acronyms)
    'TO DO list' -> 'TO DO List' (preserves all-caps)
    """
    if acronyms is None:
        acronyms = set()
    words = text.split()
...
def format_display_name(name: str, strip_prefix: bool = True, acronyms: set = None, overrides: dict = None) -> str:
    """
    ...
    return capitalize_with_acronyms(display, acronyms)
...
'''
    ...
    def parse_frontmatter(self, content: str) -> tuple[Dict[str, Any], str]:
        ...
        # Handle % prefix frontmatter (HackWiki format)
        if content.startswith('%'):
            frontmatter = {}
            lines = content.split('\n')
            content_start = 0
    ...
    def format_category_name(self, name: str) -> str:
        ...
        if not name:
            return ''
        return format_display_name(name, self.strip_prefix, self.acronyms, self.display_overrides)
    ...
    def get_page_hierarchy(self, file_path: Path, root_dir: Path) -> Dict[str, Any]:
        ...
        levels = [format_display_name(p, self.strip_prefix, self.acronyms, self.display_overrides) for p in parts]  # Display names
    ...
    def process_markdown_file(self, file_path: Path, root_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            title = (
                ...
                format_display_name(file_path.stem, self.strip_prefix, self.acronyms, self.display_overrides)
            )
    ...
    def scan_directory(self, content_dir: Path) -> List[Dict[str, Any]]:
        ...
        exclude_folders = self.config.get('paths', {}).get('exclude_folders', [
        ...
        for f in content_dir.rglob('*.md'):
            ...
            if any(excluded in f.parts for excluded in exclude_folders):
                continue
    ...
    def generate_navigation_html_for_depth(self, depth: int = 0) -> str:
        ...
        def render_sublevel(node, parent_id, level_depth, url_prefix):
            ...
            pages = sorted(node.get('__pages__', []), key=lambda p: p['title'])
        ...
        root_pages = sorted(tree.get('__pages__', []), key=lambda p: p['title'])
...
'''
    ...
    def generate_index(self, template: str, navigation_html: str, site_config_js: str, 
        ...
        replacements = {
            ...
            '{{DYNAMIC_PAGE_DATA}}': site_config_js + page_data_js + "window.currentPageId = 'welcome';",
            ...
            '{{TOTAL_CATEGORIES}}': str(len(self.categories)),
            '{{HOME_URL}}': './index.html',
            '{{ASSETS_PATH}}': 'assets/',
    ...
    def generate_page(self, page: Dict[str, Any], template: str, site_config_js: str, 
        ...
        navigation_html = self.generate_navigation_html_for_depth(depth)
        ...
        replacements = {
            ...
            '{{SITE_DESCRIPTION}}': self.description,
            '{{NAVIGATION_HTML}}': navigation_html,
            '{{DYNAMIC_PAGE_DATA}}': site_config_js + page_data_js + f"window.currentPageId = '{page['id']}';",
            '{{GENERATION_DATE}}': generation_date,
            '{{CURRENT_YEAR}}': str(current_year),
            '{{SITE_NAME_INITIAL}}': site_name_initial,
    ...
    def generate_static_pages(self, pages_dir: Path, template: str, navigation_html: str, 
        ...
        for md_file in md_files:
            ...
            replacements = {
                ...
                '{{SITE_DESCRIPTION}}': self.description,
                '{{NAVIGATION_HTML}}': navigation_html,
                '{{DYNAMIC_PAGE_DATA}}': site_config_js + page_data_js + f"window.currentPageId = '{md_file.stem}';",
                '{{GENERATION_DATE}}': generation_date,
                '{{CURRENT_YEAR}}': str(current_year),
                '{{SITE_NAME_INITIAL}}': site_name_initial,
                '{{DYNAMIC_CONTENT_PLACEHOLDER}}': page_content,
...
def main():
    ...
    parser.add_argument('--template', dest='template_dir', help='Template name (e.g., hackwiki) or path')
    ...
    if args.template_dir:
        # Check if it's a template name (e.g., "hackwiki") or a full path
